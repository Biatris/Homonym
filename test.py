...
for train_index, test_index in rs.split(pos_global_features.values):
    ...
    print(r)
    print(type(r))
    #dict(zip(Xtot.columns, mysgd.coef_[0])), X_train, y_train, n_repeats=30, random_state=1
    #perm_feat_importance = perm_feat_importance.append(pd.Series(r.importances_mean), ignore_index = True)
